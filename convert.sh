...
psql --user $POSTGRES_USER --dbname $POSTGRES_DBNAME -A -F"," -c 'SELECT postcode, ST_Y(geometry) as lat, ST_X(geometry) as lon FROM gb_postcode' > $WORKDIR/gb_postcodes.csv
